...
- **Персонажи и скиллы.** Сохраняемые system-промпты («Python-разработчик») выбираются
  в popup и подмешиваются к скрытому промпту. Скиллы — шаблоны задач с аргументами;
  отдаются по MCP как prompts, поэтому видны в клиентах как слэш-команды.
- **XML-теги.** Кроме JSON вызов можно записать тегами
  (`<dsbridge-tool name="...">`) — проще для модели, меньше ломается на кавычках.
- **Python.** Инструмент `python` выполняет код нативным интерпретатором — без
  shell-экранирования, с аргументами и stdin.
