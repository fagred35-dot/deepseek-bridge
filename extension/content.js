...
(() => {
  ...
  // Сигнатура вызова -> когда его в последний раз выполняли. Нужно только чтобы
  // не запустить один и тот же вызов дважды подряд: чат перерисовывает сообщения
  // (прокрутка, стриминг), узлы создаются заново, метка data-dsbDone пропадает —
  // и без этого короткого окна старая команда выполнялась бы повторно.
  //
  // ВАЖНО: это окно по времени, а не накопительный счётчик. Новый вызов с той же
  // командой (модель перечитывает файл после правки) должен выполняться — раньше
  // он ошибочно считался повтором и требовал кнопку «выполнить всё равно».
  ...
  // Не запускали ли этот же вызов только что? Окно короткое (10 с), поэтому оно
  // ловит только перерисовку одного блока, но НЕ мешает модели снова попросить ту
  // же команду позже. Накопительного счётчика больше нет — именно он блокировал
  // законные новые вызовы и требовал кнопку «выполнить всё равно».
  function ranRecently(sig) {
    const at = recentRuns.get(sig);
    return at != null && Date.now() - at < REPEAT_WINDOW_MS;
  }

  // Убираем из карты записи старше окна, иначе она копила бы сигнатуры всей сессии.
  function pruneRecentRuns() {
    const now = Date.now();
    for (const [key, at] of recentRuns) {
      if (now - at >= REPEAT_WINDOW_MS) recentRuns.delete(key);
    }
  }
  ...
  // Мгновенная перерисовка того же блока: команда только что уходила в мост.
  // Прячем сырой JSON компактной пометкой. Никакой кнопки «выполнить всё равно»
  // больше нет — если команда реально нужна снова, модель просто пришлёт её в
  // следующем сообщении, и оно выполнится (окно к тому времени истечёт).
  function renderRepeatCard(codeEl, spec) {
    ...
    card.append(el("span", "dsb-repeat-text", "вызов " + what + " уже отправлен — дубль из перерисовки пропущен"));
    anchor.after(card);
  ...
  function scan(manual = false) {
    ...
    nodes.forEach((node) => {
      ...
      // Тот же вызов уходил в мост только что — это перерисовка того же блока,
      // а не новая команда. Окно короткое, поэтому законные повторные вызовы
      // (модель перечитывает файл после правки) проходят.
      if (ranRecently(sig)) {
        node.dataset.dsbDone = "1";
        renderRepeatCard(node, spec);
        return;
      }
      recentRuns.set(sig, Date.now());
      pruneRecentRuns();
